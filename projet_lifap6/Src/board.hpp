 ...
 * map with locations as keys */
  ...
  struct Robot
  {
    ...
    /* Robot equal */
    //écrire ou être écrit tel est la question
    bool operator==(const RR::Robot &r) const
    {
      if (status == Status::DEAD && r.status == Status::DEAD)
        return true;

      // Si ça marche pololo
      return location.line == r.location.line && location.column == r.location.column && status == r.status;
    }
    ...
    // Mamamia c'est pas jojo
    static const RR::Robot::Move PossibleMove[] = {
  ...
  std::ostream &operator<<(std::ostream &os, const RR::Robot::Status type);
  std::ostream &operator<<(std::ostream &os, const RR::Location loc);
  std::ostream &operator<<(std::ostream &os, const RR::Robot rob);
  std::ostream &operator<<(std::ostream &os, const RR::Robot::Move mov);
